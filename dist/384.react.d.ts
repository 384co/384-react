...
export interface ChannelKeyStrings {
	encryptionKey: string;
	guestKey?: string;
	ownerKey: string;
	signKey: string;
...
declare namespace SnackabraTypes {
	type ChannelMessage = ChannelMessageTypeAlias;
	type SBServer = SBServerTypeAlias;
	type ChannelEndpoint = ChannelEndpointTypeAlias;
	type SBChannelHandle = SBChannelHandleTypeAlias;
	type ChannelSocket = ChannelSocketTypeAlias;
	type SB384 = SB384TypeAlias;
	type SBObjectHandle = SBObjectHandleTypeAlias;
	type SBPayload = SBPayloadAlias;
}
export interface ChannelStoreType {
	id: any;
	key: any;
	alias: any;
	socket: SnackabraTypes.ChannelSocket;
	capacity: any;
	motd: any;
	owner: any;
	status: any;
	messages: SnackabraTypes.ChannelMessage[];
	getMessages: () => SnackabraTypes.ChannelMessage[];
...
declare class ChannelStore implements ChannelStoreType {
	...
	constructor(config: SBServerTypeAlias, channelId?: string);
	private getChannel;
	private makeVisible;
	private save;
	...
	getStorageAmount: () => Promise<number | null>;
	processOldMessages: (messages: Array<SnackabraTypes.ChannelMessage>) => Promise<void>;
	batchSet: (messages: Array<{
		key: string;
		value: any;
	}>) => Promise<void>;
	getOldMessages: (length?: number, size?: number) => Promise<Map<string, any>>;
	...
	receiveMessage: (m: ChannelMessage, updateState?: boolean) => void;
}
...
declare class SnackabraStore implements SnackabraStoreType {
	...
	SB: Snackabra;
	ready: Promise<unknown>;
	constructor(sbConfig: SnackabraTypes.SBServer);
	private save;
