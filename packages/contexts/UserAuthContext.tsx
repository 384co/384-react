...
export function UserAuthProvider(props: UserAuthProviderProps) {
  ...
  const [channel, setUserChannel] = useState<ChannelStoreType | null>(null);
  ...
  const connect = async (channelId: string) => {
    let _channel = SB!.channels[channelId] as ChannelStoreType
    if (_channel) {
  ...
  const joinOrConnect = async () => {
    ...
    if (SB && channelId) {
      if (await connect(channelId)) {
        ...
        const c = await SB.joinChannel(user.channelId, user.key)
        console.log('joined')
        setUserChannel(c)
        localStorage.setItem('channelId', user.channelId)
        return true
  ...
  const register = async (email: string, password: string): Promise<boolean> => {
    ...
    try {
      ...
      const channelEndpoint = new __.NewSB.ChannelEndpoint(SB!.config, wallet.identity, wallet.id as string)
      const SBServer = new __.NewSB.Snackabra(SB!.config)
      const c = await SBServer.create(SB!.config, channelEndpoint)
      if (c) {
